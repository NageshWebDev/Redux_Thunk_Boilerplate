...
import { fetchData } from "../feature/ThunkSliceB";

export default function ChildThunkB() {
  const [clicked, setClicked] = useState(false);
  const dispatch = useDispatch();
  const loading = useSelector((state) => state.thunkReducerB.loading);
  const status = useSelector((state) => state.thunkReducerB.status);
  const type = useSelector((state) => state.thunkReducerB.type);
  const { ok, message } = useSelector((state) => state.thunkReducerB.response);

  function onClickHandler() {
    setClicked(true);
    dispatch(fetchData(2000));
  }
